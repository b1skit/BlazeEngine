...
//	
//	// Falloff:
//	float distanceToLight		= length((lightWorldPos - worldPosition));
////	float falloffFactor			= 1.0 - min(distanceToLight / attenuationDist, 1.0);
//	float falloffFactor			= 1.0 / max((distanceToLight * distanceToLight), 0.1);
//	// ^^ Pretty sure this isn't right...
//
//	// Final result:
//	FragColor = ((diffuseContribution + specContribution) * shadowFactor);
	
	// DEBUG: Just output the light color
